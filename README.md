...
Приложение на Next.js 14 повторяет ключевые блоки анкеты «ТЗ NEW с дизайнером», собирает данные клиента, формирует краткое ТЗ (до 1000 символов) через OpenAI и генерирует референсы по комнатам на основе Gemini (Imagen 3 или vision-моделей). Все введённые данные можно выгрузить в DOCX/PDF, а планировки хранятся на сервере.
...
2. **Переменные окружения** – создайте `.env.local`:
   ...
   OPENAI_API_KEY=sk-...
   GOOGLE_API_KEY=...
   GEMINI_IMAGE_MODEL=imagen-3.0 # опционально
   GEMINI_VISION_MODEL=gemini-2.0-flash # опционально
   ```
3. **Запуск разработки**
   ```bash
   npm run dev
...
| `/api/refs` | POST | Принимает объект комнаты (тип, площадь, заметки), стиль, материалы и ограничения. Возвращает до 4 PNG (base64) и метаданные о модели/режиме (vision или imagen). |
...
- Без плана используется Imagen 3 (`imagen-3.0`). В обоих случаях фронт получает массив base64 PNG и метаданные о модели.
